...
from metrics import get_cindex, get_rm2, roc_auc_score
from dataset import *
from model import MGraphDTA, GraphDTA_GCN, GraphDTA_GIN
from utils import *
...
def main():
    ...
    parser.add_argument('--model_name', required=True, help='model name: gcn/gin/mgraphdta')
    ...
    device = torch.device('cuda:0')
    if args.model_name == 'mgraphdta':
        model = MGraphDTA(2, 25 + 1, embedding_size=128, filter_num=32, out_dim=1).to(device)
    elif args.model_name == 'gcn':
        model = GraphDTA_GCN(1, 32, embed_dim=128, num_features_xd=78, num_features_xt=25).to(device)
    elif args.model_name == 'gin':
        model = GraphDTA_GIN(1, 32, embed_dim=128, num_features_xd=18, num_features_xt=25).to(device)
    else:
