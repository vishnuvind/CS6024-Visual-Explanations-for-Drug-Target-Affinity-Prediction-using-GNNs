import numpy as np
from sklearn.metrics import r2_score, roc_auc_score, cohen_kappa_score
...
def sparsity(attention, threshold):
    mask = attention>threshold
    activated = float(sum(mask))
    total = float(len(mask))
    sparsity =  1.0 - activated/total
    return sparsity


get_r2_score = r2_score
